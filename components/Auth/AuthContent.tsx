...
function AuthContent({ isLogin, onAuthenticate }: IAuthContent) {
    ...
    const [credentialsInvalid, setCredentialsInvalid] = useState<ICredentialsInvalid>({
        email: false,
        password: false,
    });
    ...
    return (
        <SafeAreaView style={{flex: 1}}>
            <View style={styles.root}>
                <Text style={styles.title}>{isLogin ? 'Log In to Continue!' : 'Sign Up to Continue!'}</Text>
                {/* TODO: add auth with socials */}
                <View>
                    <Button style={styles.socialBtn} isDisabled isCentered={false} size="large" shape="rounded">
                ...
                <View style={styles.switchContainer}>
                    <Text style={styles.switchText}>{isLogin ? "Don't have an account?" : "Have an account?"}</Text>
                    <Link onPress={switchAuthModeHandler} textStyle={styles.switchLinkText}>{isLogin ? "Sign Up" : "Login"}</Link>
                </View>
            </View>
        </SafeAreaView>
    )
